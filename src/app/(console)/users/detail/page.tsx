...
export default function UserDetailPage() {
  ...
  return <div className="mx-auto w-full max-w-[1720px] px-4 py-6 lg:px-6 lg:py-7">
    ...
    {data.enhancementsReady && hasAdminPermission(admin.role, "users.moderate") ? <section className="mt-6 rounded-xl border border-border/80 bg-card/35 p-4" aria-labelledby="moderate-user-title"><h2 id="moderate-user-title" className="text-base font-semibold">사용자 조치</h2><div className="mt-5 w-full"><UserModerationForm key={`${user.id}:${user.accountStatus}`} userId={user.id} accountStatus={user.accountStatus} /></div></section> : !data.enhancementsReady ? <p className="mt-4 text-xs text-warning">사용자 경고·정지·해제는 관리자 마이그레이션 적용 후 활성화됩니다.</p> : null}
  </div>;
}
