...
export default async function ModerationDetailPage({ params }: { params: Promise<{ reportId: string }> }) {
  ...
  return <div className="mx-auto w-full max-w-[1200px] px-4 py-6 lg:px-6 lg:py-7">
    <PageHeader context={<Link href="/inquiries?tab=reports" className="inline-flex items-center gap-1 hover:underline"><ArrowLeft className="size-3" />신고 내역</Link>} title={`${reportTargetLabel(report.targetType)} 신고`} description={report.reason} status={<AdminStatusBadge label={reportStatusLabel(report.status)} tone={statusTone(report.status)} />} metadata={<><span>접수 {formatKoreaDateTime(report.createdAt)}</span><span>같은 대상 누적 {formatNumber(report.cumulativeReportCount)}건</span></>} />
