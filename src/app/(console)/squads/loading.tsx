...
function PlayerMetricSkeleton() {
  return (
    <div className="bg-card/45 p-3 md:p-4">
...
function PlayerMobileRowSkeleton({ secondary = true }: { secondary?: boolean }) {
  return (
    <div className="p-4">
      <div className="flex items-start justify-between gap-3">
        <div className="space-y-1.5">
          <Skeleton className="h-5 w-24" />
          {secondary ? <Skeleton className="h-3 w-20" /> : null}
        </div>
...
export default function SquadsLoading() {
  return (
    <div className="mx-auto w-full max-w-[1720px] px-4 py-6 lg:px-6 lg:py-7" aria-busy="true" aria-label="선수 관리 정보를 불러오는 중">
      <header className="flex items-center justify-between gap-4 border-b border-border/70 pb-4 sm:items-end">
        <Skeleton className="h-9 w-32" />
        <Skeleton className="h-11 w-32 rounded-lg" />
      </header>

      <section className="mt-6 grid grid-cols-2 gap-px overflow-hidden rounded-xl border border-border/80 bg-border/70">
      ...
      <section className="mt-6 overflow-hidden rounded-xl border border-border/80 bg-card/35">
        ...
        <div className="flex flex-col gap-2 border-b border-border/70 p-3 lg:flex-row lg:items-center">
          ...
          <div className="grid w-full grid-cols-2 gap-2 lg:ml-auto lg:flex lg:w-auto lg:items-center">
            <Skeleton className="h-11 w-full rounded-xl lg:w-44" />
            <Skeleton className="h-11 w-full rounded-xl lg:w-44" />
          </div>
        </div>

        <div className="divide-y divide-border/70 md:hidden">
          {Array.from({ length: 8 }).map((_, index) => <PlayerMobileRowSkeleton key={index} secondary={index % 3 !== 0} />)}
        </div>
