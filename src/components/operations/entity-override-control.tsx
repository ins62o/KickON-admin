...
export function EntityOverrideControl({ entityType, entityId, overrides, currentValues, canEdit, openApply = false, defaultReason = "" }: {
  entityType: EntityType; entityId: string; overrides: EntityOverrideRecord[]; currentValues: Record<string, unknown>; canEdit: boolean; openApply?: boolean; defaultReason?: string;
}) {
  const active = overrides.filter((item) => !item.releasedAt);
  return <div className="space-y-4"><ApplyDialog entityType={entityType} entityId={entityId} currentValues={currentValues} canEdit={canEdit} open={openApply && canEdit} defaultReason={defaultReason} /><div className="space-y-2">{active.length > 0 ? active.map((item) => <ActiveOverride key={item.id} item={item} canEdit={canEdit} />) : <p className="rounded-lg border border-dashed border-border px-3 py-5 text-center text-xs text-muted-foreground">보호 중인 직접 수정값이 없습니다.</p>}</div>{!canEdit ? <p className="text-[11px] leading-5 text-muted-foreground">직접 수정은 관리자 권한이 필요합니다.</p> : null}</div>;
}

function ApplyDialog({ entityType, entityId, currentValues, canEdit, open, defaultReason }: { entityType: EntityType; entityId: string; currentValues: Record<string, unknown>; canEdit: boolean; open: boolean; defaultReason: string }) {
  ...
  return <Dialog defaultOpen={open}><DialogTrigger asChild><Button className="w-full" disabled={!canEdit}><LockKeyhole className="size-4" /> 직접 수정</Button></DialogTrigger><DialogContent className="sm:max-w-md"><DialogHeader><DialogTitle>{entityType === "fixture" ? "경기" : "순위"} 정보 직접 수정</DialogTitle><DialogDescription>직접 수정한 값은 보호를 해제하기 전까지 외부 데이터 새로고침으로 바뀌지 않습니다.</DialogDescription></DialogHeader><form action={action} className="space-y-4"><input type="hidden" name="entityId" value={entityId} /><div className="space-y-1.5"><label htmlFor={`${entityType}-override-field`} className="text-xs font-medium">수정할 항목</label><Select name="field" value={field} onValueChange={setField}><SelectTrigger id={`${entityType}-override-field`} className="w-full"><SelectValue /></SelectTrigger><SelectContent>{fields.map((item) => <SelectItem key={item.value} value={item.value}>{item.label}</SelectItem>)}</SelectContent></Select></div><ValueInput option={selected} value={currentValues[field]} /><div className="space-y-1.5"><label htmlFor={`${entityType}-override-reason`} className="text-xs font-medium">수정 이유</label><Textarea id={`${entityType}-override-reason`} name="reason" defaultValue={defaultReason} minLength={3} maxLength={1000} required placeholder="공식 기록 또는 사용자 제보 등 근거를 기록하세요." /></div>{state.message ? <p role="status" className={state.status === "success" ? "text-xs text-emerald-300" : "text-xs text-rose-300"}>{state.message}</p> : null}<DialogFooter className="mx-0 mb-0 rounded-lg px-0 pb-0"><DialogClose asChild><Button type="button" variant="outline">취소</Button></DialogClose><Button type="submit" disabled={pending}>{pending ? <LoaderCircle className="size-4 animate-spin" /> : <Save className="size-4" />}{pending ? "저장 중" : "수정값 저장"}</Button></DialogFooter></form></DialogContent></Dialog>;
}

function ValueInput({ option, value }: { option: FieldOption; value: unknown }) {
  if (option.kind === "status") return <div className="space-y-1.5"><label htmlFor="override-value-status" className="text-xs font-medium">수정값</label><Select name="value" defaultValue={typeof value === "string" ? value : "SCHEDULED"}><SelectTrigger id="override-value-status" className="w-full"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="SCHEDULED">예정</SelectItem><SelectItem value="LIVE">진행 중</SelectItem><SelectItem value="FINISHED">종료</SelectItem><SelectItem value="CANCELED">취소</SelectItem></SelectContent></Select></div>;
  ...
  return <div className="space-y-1.5"><label htmlFor={`override-value-${option.value}`} className="text-xs font-medium">수정값</label><Input key={option.value} id={`override-value-${option.value}`} name="value" type={option.kind === "datetime" ? "datetime-local" : "number"} defaultValue={inputValue} step={option.step} required /></div>;
}
...
function ActiveOverride({ item, canEdit }: { item: EntityOverrideRecord; canEdit: boolean }) {
  ...
  const label = fields.find((field) => field.value === item.fieldPath)?.label ?? item.fieldPath;
