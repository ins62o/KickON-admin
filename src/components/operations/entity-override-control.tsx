...
export function EntityOverrideControl({ entityType, entityId, overrides, currentValues, canEdit, openApply = false, defaultReason = "", triggerLabel = "직접 수정", dialogTitle, dialogDescription, submitLabel = "수정값 저장", triggerClassName = "w-full", showTriggerIcon = true, showSubmitIcon = true, showActiveOverrides = true }: {
  entityType: EntityType; entityId: string; overrides: EntityOverrideRecord[]; currentValues: Record<string, unknown>; canEdit: boolean; openApply?: boolean; defaultReason?: string; triggerLabel?: string; dialogTitle?: string; dialogDescription?: string | false; submitLabel?: string; triggerClassName?: string; showTriggerIcon?: boolean; showSubmitIcon?: boolean; showActiveOverrides?: boolean;
}) {
  const active = overrides.filter((item) => !item.releasedAt);
  return <div className={showActiveOverrides ? "space-y-4" : undefined}><ApplyDialog entityType={entityType} entityId={entityId} currentValues={currentValues} canEdit={canEdit} open={openApply && canEdit} defaultReason={defaultReason} triggerLabel={triggerLabel} dialogTitle={dialogTitle} dialogDescription={dialogDescription} submitLabel={submitLabel} triggerClassName={triggerClassName} showTriggerIcon={showTriggerIcon} showSubmitIcon={showSubmitIcon} />{showActiveOverrides ? <><div className="space-y-2">{active.length > 0 ? active.map((item) => <ActiveOverride key={item.id} item={item} canEdit={canEdit} />) : <p className="rounded-lg border border-dashed border-border px-3 py-5 text-center text-xs text-muted-foreground">보호 중인 직접 수정값이 없습니다.</p>}</div>{!canEdit ? <p className="text-[11px] leading-5 text-muted-foreground">직접 수정은 관리자 권한이 필요합니다.</p> : null}</> : null}</div>;
}
