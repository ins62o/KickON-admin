...
function ValueInput({ option, value }: { option: FieldOption; value: unknown }) {
  ...
  if (option.kind === "datetime") return <DateTimeValueInput value={typeof value === "string" ? value : ""} />;
  const inputValue = value === null || value === undefined ? "" : String(value);
  return <div className="space-y-2.5"><label htmlFor={`override-value-${option.value}`} className="block text-sm font-semibold text-foreground">수정값</label><Input key={option.value} id={`override-value-${option.value}`} name="value" type="number" defaultValue={inputValue} step={option.step} required className="h-11 rounded-xl px-3.5 text-sm" /></div>;
}

function DateTimeValueInput({ value }: { value: string }) {
  const initial = value ? koreaDateTimeInput(value).split("T") : ["", ""];
  const [date, setDate] = useState(initial[0] ?? "");
  const [time, setTime] = useState(initial[1] ?? "");
  return <div className="space-y-2.5">
    <p className="text-sm font-semibold text-foreground">수정값</p>
    <input type="hidden" name="value" value={`${date}T${time}`} />
    <ScheduleDateTimePicker id="fixture-override-kickoff" date={date} time={time} onDateChange={setDate} onTimeChange={setTime} />
  </div>;
}
