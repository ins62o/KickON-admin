...
export function ReportStatusForm({ reportId, status }: { reportId: string; status: string }) {
  ...
  return <form action={action} className="space-y-4"><input type="hidden" name="reportId" value={reportId} /><label className="block text-xs font-medium">신고 처리 상태<select name="status" defaultValue={status} className="mt-1.5 h-9 w-full rounded-md border border-input bg-background px-3 text-xs"><option value="OPEN">접수</option><option value="REVIEWED">검토 중</option><option value="RESOLVED">조치 완료</option><option value="DISMISSED">기각</option></select></label><label className="block text-xs font-medium">처리 메모<Textarea name="resolutionNote" maxLength={2000} rows={3} className="mt-1.5 text-xs" placeholder="완료·기각 시 필수" /></label><label className="block text-xs font-medium">변경 사유<Textarea name="reason" required minLength={3} maxLength={1000} rows={2} className="mt-1.5 text-xs" placeholder="감사 로그에 남길 사유" /></label><ActionMessage state={state} /><div className="flex justify-end"><ActionSubmit>처리 상태 저장</ActionSubmit></div></form>;
}
...
export function ContentVisibilityForm({ reportId, targetType, targetId, hidden }: { reportId: string; targetType: string; targetId: string; hidden: boolean }) {
  ...
  return <form action={action} className="space-y-4"><input type="hidden" name="reportId" value={reportId} /><input type="hidden" name="targetType" value={targetType} /><input type="hidden" name="targetId" value={targetId} /><input type="hidden" name="hidden" value={hidden ? "false" : "true"} /><p className="text-xs leading-5 text-muted-foreground">{hidden ? "원문 노출을 복원합니다." : "원문은 삭제하지 않고 모바일·웹의 일반 조회에서 숨깁니다."}</p><label className="block text-xs font-medium">{hidden ? "복원" : "숨김"} 사유<Textarea name="reason" required minLength={3} maxLength={1000} rows={3} className="mt-1.5 text-xs" /></label><ActionMessage state={state} /><div className="flex justify-end"><ActionSubmit variant={hidden ? "outline" : "destructive"}>{hidden ? "콘텐츠 복원" : "콘텐츠 숨김"}</ActionSubmit></div></form>;
}
