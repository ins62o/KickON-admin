...
export function ReportStatusForm({ reportId, status }: { reportId: string; status: string }) {
  ...
  return <form action={action} className="space-y-4"><input type="hidden" name="reportId" value={reportId} /><label className="block text-xs font-medium">신고 처리 상태<select name="status" defaultValue={visibleStatus} className="mt-1.5 h-9 w-full rounded-md border border-input bg-background px-3 text-xs"><option value="OPEN">새 문의</option><option value="RESOLVED">답변 완료</option></select></label><label className="block text-xs font-medium">처리 메모<Textarea name="resolutionNote" maxLength={2000} rows={3} className="mt-1.5 text-xs" placeholder="답변 완료 시 필수" /></label><label className="block text-xs font-medium">변경 사유<Textarea name="reason" required minLength={3} maxLength={1000} rows={2} className="mt-1.5 text-xs" placeholder="감사 로그에 남길 사유" /></label><ActionMessage state={state} /><div className="flex justify-end"><ActionSubmit>처리 상태 저장</ActionSubmit></div></form>;
}
