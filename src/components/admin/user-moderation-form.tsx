...
export function UserModerationForm({ userId, suspended, reportId }: { userId: string; suspended: boolean; reportId?: string }) {
  ...
  return <form action={action} className="space-y-4">
    ...
    <label className="block text-xs font-medium">조치<select name="action" defaultValue={suspended ? "UNSUSPEND" : "WARN"} className="mt-1.5 h-9 w-full rounded-md border border-input bg-background px-3 text-xs"><option value="WARN">경고 기록</option><option value="SUSPEND">커뮤니티 활동 정지</option>{suspended ? <option value="UNSUSPEND">정지 해제</option> : null}</select></label>
    <label className="block text-xs font-medium">정지 종료 시각<Input type="datetime-local" name="suspendedUntil" className="mt-1.5 text-xs" /><span className="mt-1 block text-[10px] font-normal text-muted-foreground">정지 조치를 선택한 경우에만 사용합니다. 최대 365일입니다.</span></label>
