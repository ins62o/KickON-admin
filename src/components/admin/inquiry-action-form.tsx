...
export function InquiryActionForm({ inquiryId, status, answer }: { inquiryId: string; status: string; answer: string | null }) {
  ...
  return <form action={action} className="space-y-4">
    ...
    <label className="block text-xs font-medium">처리 상태<select name="status" defaultValue={status} className="mt-1.5 h-9 w-full rounded-md border border-input bg-background px-3 text-xs"><option value="RECEIVED">접수</option><option value="IN_PROGRESS">처리 중</option><option value="ANSWERED">답변 완료</option><option value="CLOSED">종료</option></select></label>
