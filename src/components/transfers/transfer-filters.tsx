...
export function TransferFilters({ values, teams }: { values: FilterValues; teams: Array<{ id: string; name: string }> }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2" aria-label="선수 변동 필터">
      ...
      <Select value={values.team} onValueChange={(value) => change("team", value)} disabled={pending}><SelectTrigger className="w-44"><SelectValue placeholder="전체 구단" /></SelectTrigger><SelectContent><SelectItem value="all">전체 구단</SelectItem>{teams.map((team) => <SelectItem key={team.id} value={team.id}>{team.name}</SelectItem>)}</SelectContent></Select>
