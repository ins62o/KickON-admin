...
모바일 저장소와 관리자 저장소가 과거 서로 다른 내용에 `202608300001`~`202608300007`을 사용했습니다. 이 저장소에서는 기존 관리자 전용 migration을 전역 고유 순서 `202609050001`~`202609050013`으로 조정했고, `202609070001`에서 동기화 복구를 차단하는 선수 삭제 RPC, `202609070002`에서 경기별 경기장·직관 인증 좌표와 반경, `202609070003`에서 경기 날짜·경기장·인증 위치 통합 수정 RPC, `202609070005`에서 선수 상세 전체 필드 수정과 수동 보호값 RPC를 추가합니다. 직관 인증은 수정된 경기 날짜와 경기별 좌표를 우선하고 값이 없으면 선택된 경기장의 좌표를 사용합니다. `202609050001`~`007`은 운영 기반, `008`은 역할·capability와 핵심 데이터 센터 계약, `009`~`012`는 대시보드·사용자 조회 보정, `013`은 DB·Storage aggregate 사용량 RPC입니다. 파일명 순서대로 적용하되, 먼저 원격 `schema_migrations`와 실제 객체를 감사해 과거 충돌 이력을 통합해야 합니다.
