...
do $$ begin create type public.admin_role as enum ('viewer', 'operator', 'admin', 'super_admin'); exception when duplicate_object then null; end $$;
do $$ begin create type public.report_status as enum ('open', 'in_review', 'resolved', 'rejected', 'on_hold'); exception when duplicate_object then null; end $$;
do $$ begin create type public.report_priority as enum ('low', 'normal', 'high', 'urgent'); exception when duplicate_object then null; end $$;
do $$ begin create type public.sync_run_status as enum ('queued', 'running', 'succeeded', 'partial', 'failed', 'cancelled'); exception when duplicate_object then null; end $$;
do $$ begin create type public.error_group_status as enum ('open', 'investigating', 'resolved', 'ignored'); exception when duplicate_object then null; end $$;
...
create index if not exists error_groups_queue_idx
...
create index if not exists error_events_group_occurred_idx
...
create index if not exists error_events_release_occurred_idx
...
create index if not exists sync_runs_job_created_idx on public.sync_runs (job_key, created_at desc);
create index if not exists sync_runs_status_created_idx on public.sync_runs (status, created_at desc);
...
create index if not exists sync_run_items_run_idx on public.sync_run_items (run_id, id);
...
drop trigger if exists user_data_reports_set_updated_at on public.user_data_reports;
create trigger user_data_reports_set_updated_at before update on public.user_data_reports
  for each row execute function public.set_admin_updated_at();
drop trigger if exists error_groups_set_updated_at on public.error_groups;
...
create trigger sync_runs_set_updated_at before update on public.sync_runs
  for each row execute function public.set_admin_updated_at();
drop trigger if exists manual_overrides_set_updated_at on public.manual_overrides;
...
drop trigger if exists user_data_reports_record_status on public.user_data_reports;
...
drop trigger if exists user_data_reports_record_initial_status on public.user_data_reports;
...
declare
  ...
  audit_reason text;
begin
  select administrator.role into actor_admin_role
  from public.admin_users administrator
...
drop trigger if exists user_data_reports_audit on public.user_data_reports;
create trigger user_data_reports_audit after update on public.user_data_reports
  for each row execute function public.record_admin_audit_log();
drop trigger if exists error_groups_audit on public.error_groups;
create trigger error_groups_audit after update on public.error_groups
  for each row execute function public.record_admin_audit_log();
drop trigger if exists manual_overrides_audit on public.manual_overrides;
...
drop policy if exists "super admins create memberships" on public.admin_users;
...
drop policy if exists "super admins update memberships" on public.admin_users;
...
drop policy if exists "super admins delete memberships" on public.admin_users;
...
drop policy if exists "users read own data reports" on public.user_data_reports;
...
drop policy if exists "operators update data reports" on public.user_data_reports;
...
drop policy if exists "admins read report history" on public.user_data_report_history;
...
drop policy if exists "admins read error groups" on public.error_groups;
create policy "admins read error groups" on public.error_groups
  for select to authenticated using (public.is_admin('viewer'));
drop policy if exists "operators update error groups" on public.error_groups;
create policy "operators update error groups" on public.error_groups
  for update to authenticated using (public.is_admin('operator')) with check (public.is_admin('operator'));
drop policy if exists "admins read error events" on public.error_events;
...
create policy "admins read sync runs" on public.sync_runs
  for select to authenticated using (public.is_admin('viewer'));
drop policy if exists "operators create sync runs" on public.sync_runs;
...
drop policy if exists "operators update sync runs" on public.sync_runs;
create policy "operators update sync runs" on public.sync_runs
  for update to authenticated using (public.is_admin('operator')) with check (public.is_admin('operator'));
drop policy if exists "admins read sync run items" on public.sync_run_items;
create policy "admins read sync run items" on public.sync_run_items
  for select to authenticated using (public.is_admin('viewer'));

drop policy if exists "admins read overrides" on public.manual_overrides;
create policy "admins read overrides" on public.manual_overrides
  for select to authenticated using (public.is_admin('viewer'));
drop policy if exists "admins create overrides" on public.manual_overrides;
...
drop policy if exists "admins update overrides" on public.manual_overrides;
...
drop policy if exists "admins delete overrides" on public.manual_overrides;
