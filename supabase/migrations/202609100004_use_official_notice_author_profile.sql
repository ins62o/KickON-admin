...
-- Notices are operated by an email-only console account, but the mobile app
-- expects every author to have a complete community profile. Attribute notices
-- to the existing official "관리자" Incheon profile while auditing the real
-- console actor separately.
...
begin
  ...
  select profile.id
  into notice_author_id
  from public.profiles profile
  where profile.nickname = '관리자'
    and profile.team_id = 'incheon'
  limit 1;

  if notice_author_id is null then
    raise exception 'OFFICIAL_NOTICE_AUTHOR_REQUIRED';
  end if;
  ...
  insert into public.posts (
    ...
    normalized_content, notice_author_id, '{}', 'VISIBLE'
  )
...
comment on function public.admin_create_community_notice(text, text, text, text) is
  'Creates an audited notice attributed to the official Incheon administrator community profile.';
