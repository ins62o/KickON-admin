...
begin
  ...
  if target_fixture.id is null
     or target_fixture.status = 'CANCELED'
...
comment on function public.send_fixture_cheer(text, text, text) is
  'Sends a fixture cheer on the Korea matchday when the signed-in profile supports the home or away team.';
