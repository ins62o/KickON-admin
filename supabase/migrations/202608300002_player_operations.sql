...
-- Apply after 202608300001_admin_operations.sql.

alter table public.manual_overrides
  add column season integer not null default 2026,
  add column league_id text not null default 'kleague';
...
create table public.player_squad_snapshots (
...
create index player_squad_snapshots_lookup_idx
  on public.player_squad_snapshots (team_id, season, league_id, captured_at desc);

create table public.player_squad_snapshot_rows (
...
create index player_squad_snapshot_rows_player_idx
  on public.player_squad_snapshot_rows (player_id, snapshot_id);

create table public.player_change_events (
  id uuid primary key default gen_random_uuid(),
...
create index player_change_events_queue_idx
  on public.player_change_events (review_status, detected_at desc);
create index player_change_events_player_idx
  on public.player_change_events (player_id, detected_at desc);
create index player_change_events_teams_idx
