-- League-wide notices do not require an operator-selected team. The posts table
-- still has a legacy non-null team_id column, so resolve an internal storage team
-- without exposing that implementation detail in the admin form.
...
begin
  ...
  if normalized_board = 'TEAM' then
    ...
    order by
      case when team.id = actor_team_id then 0 else 1 end,
      team.id
    limit 1;
...
comment on function public.admin_create_community_notice(text, text, text, text) is
  'Creates an audited notice; league notices resolve their required storage team internally.';
