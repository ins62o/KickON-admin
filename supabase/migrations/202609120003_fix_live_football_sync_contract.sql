-- The deployed live Edge Function consumes the provider-wide in-play feed when
-- no league scope is supplied. One polling loop updates every supported league.
-- Sending one numeric leagueId per league is rejected before sync state can be
-- recorded and also conflicts with the function's single polling lock.
...
comment on function public.invoke_live_football_sync() is
  'Polls the provider-wide in-play feed every five seconds and updates all supported leagues.';
