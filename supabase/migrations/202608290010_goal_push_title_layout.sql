...
begin
  ...
  loop
    ...
    minute_label := case
      when minute_value <= 45 then '전반' || minute_value
      when minute_value <= 90 then '후반' || (minute_value - 45)
      when minute_value <= 105 then '연장전반' || (minute_value - 90)
      else '연장후반' || (minute_value - 105)
    end;
    if added_time > 0 then
      minute_label := minute_label || '+' || added_time;
    end if;
    minute_label := minute_label || '분';
...
comment on function public.create_goal_notifications() is
  'Creates deduplicated goal pushes with the period and minute in the title.';
